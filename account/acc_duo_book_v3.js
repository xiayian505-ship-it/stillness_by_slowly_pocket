document.addEventListener("DOMContentLoaded", async ()=>{
    ...
    for(let d=1; d<=days; d++){
      ...
      day.innerHTML = `
        ...
        <span class="day-totals">
          <span class="day-total colA"><span class="day-name">${escapeHtml(names.A)}</span> ${Money.formatNumber(totals.A, {maximumFractionDigits:0})}</span>
          <span class="day-total colB"><span class="day-name">${escapeHtml(names.B)}</span> ${Money.formatNumber(totals.B, {maximumFractionDigits:0})}</span>
        </span>
      `;
          ...
          btn.onclick = ()=>{
            ...
            openEditModal({
              ...
              onOpen: ()=>{
                ...
                if(editCurrency) editCurrency.value = currentCurrency?.currency || "TWD";
  ...
  function addRecord(type){
    ...
    openEditModal({
      ...
      amount:"",
      onOpen: ()=>{
        if(editCurrency) editCurrency.value = "TWD";
  ...
  function updateAdjustColor(){
    ...
    const monthAdjust = getMonthAdjust();
    title.style.color = monthAdjust.side === "A" ? "#e58a4b" : "#8a79d6";
  }
  ...
  if(settleCurrency){
    settleCurrency.onchange = ()=>updateSummary();
  }
